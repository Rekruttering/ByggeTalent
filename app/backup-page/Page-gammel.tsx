...
const testQuestions = [
  "Min nuvaerende rolle foeles naturlig for mig",
  "Mine staerkeste sider kommer i spil i mit job",
  "De opgaver jeg sidder med giver mig energi",
  "Samarbejdet omkring mig understoetter den maade, jeg arbejder bedst paa",
  "Jeg foeler mig tryg ved at sige min mening",
  "Jeg har reel mulighed for at paavirke beslutninger i mit arbejde",
  "Tempoet og forventningerne passer til mig",
  "Jeg savner sjaeldent at bruge andre sider af mig selv i arbejdet",
  "Jeg kan se mig selv i denne rolle over laengere tid",
...
export default function Home() {
  ...
  const isStep3Valid = form.profiles.length > 0;
  const isStep4Valid = form.consent && form.gdpr;
  const isValid = isStep1Valid && isStep3Valid && isStep4Valid;

  const displayStep = step === 21 ? 2 : step;
  const totalSteps = 5;
      ...
      {!enteredApp ? (
        ...
        >
          ...
          {step === 1 && (
            <div style={stepCardStyle}>
              <div style={formGridStyle}>
                <TextInput placeholder="Navn *" value={form.name} onChange={(value) => update("name", value)} />
                <TextInput placeholder="Email *" value={form.email} onChange={(value) => update("email", value)} />
                <TextInput placeholder="Telefon *" value={form.phone} onChange={(value) => update("phone", value)} />
                <TextInput placeholder="Adresse / by / postnummer *" value={form.address} onChange={(value) => update("address", value)} />
                <TextInput placeholder="Nuvaerende titel / rolle *" value={form.currentTitle} onChange={(value) => update("currentTitle", value)} />
                <SelectInput value={form.experience} onChange={(value) => update("experience", value)} options={["Vaelg erfaring *", "0-3 aar", "4-7 aar", "8-12 aar", "12+ aar"]} />
                <TextInput placeholder="LinkedIn *" value={form.linkedin} onChange={(value) => update("linkedin", value)} />
                <SelectInput value={form.salary} onChange={(value) => update("salary", value)} options={["Vaelg loenretning *", "Under nuvaerende niveau", "Samme niveau", "Over nuvaerende niveau"]} />
                <SelectInput value={form.distance} onChange={(value) => update("distance", value)} options={["Vaelg geografisk raekkevidde *", "0-20 km", "20-50 km", "50+ km", "Hele Danmark"]} />
                <textarea style={textareaStyle} placeholder="Er der noget vigtigt vi skal vide om dig?" value={form.supplementaryInfo} onChange={(e) => update("supplementaryInfo", e.target.value)} />
              </div>

              <CvUploadCard cvFile={cvFile} applicationFile={applicationFile} onCvChange={setCvFile} onApplicationChange={setApplicationFile} />
          ...
          {step === 21 && (
            ...
            >
              <div>
                <h2 style={{ fontSize: "32px", lineHeight: 1.1, color: "#F3EFE6", margin: "0 0 10px 0", fontWeight: 700 }}>
                  Hvordan oplever du dit arbejdsliv lige nu?
                </h2>

                <p style={{ color: "rgba(243,239,230,0.78)", fontSize: "16px", lineHeight: 1.6, margin: 0 }}>
              ...
              {testQuestions.map((question, index) => (
                <div
                  ...
                  style={{
                    ...
                    paddingBottom: "2px",
                    borderBottom:
                      index === testQuestions.length - 1 ? "none" : "1px solid rgba(243,239,230,0.08)",
                  }}
                >
                  <div style={{ fontSize: "15px", lineHeight: 1.5, color: "#F3EFE6", fontWeight: 500 }}>
                    {index + 1}. {question}
                  </div>

                  <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
                    <div style={{ width: "18px", height: "18px", borderRadius: "50%", background: "#D83A34", flexShrink: 0 }} />
                    <div style={{ flex: "0 0 180px", height: "3px", background: "rgba(255,255,255,0.8)", borderRadius: "999px" }} />
                    <div style={{ width: "18px", height: "18px", borderRadius: "50%", background: "#62C84A", flexShrink: 0 }} />
                  </div>

                  <div style={{ display: "flex", gap: "34px", paddingLeft: "22px" }}>
                    {[1, 2, 3, 4, 5].map((value) => (
                      <button
                        key={value}
                        type="button"
              ...
              <div style={stepButtonsStyle}>
                ...
                {!showResult ? (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      if (isCompleted) setShowResult(true);
                    }}
              ...
              {showResult && (
                <div style={{ marginTop: "24px", color: "#F3EFE6", display: "grid", gap: "16px" }}>
                  <div>Samlet score: {totalScore}</div>

                  <h3 style={{ margin: 0, color: "#6EDC5F" }}>Din arbejdssituation lige nu</h3>

                  <p style={{ margin: 0, lineHeight: 1.6 }}>
                    Din score viser, at du stadig finder vaerdi i dit nuvaerende arbejde, men der er tydelige omraader, hvor du savner mere mening, udvikling eller bedre rammer. Det kan vaere et tegn paa, at du er klar til naeste skridt.
                  </p>
                </div>
              )}
            </div>
          ...
          {step === 3 && (
            <div style={stepCardStyle}>
              ...
              <div style={stepButtonsStyle}>
                <button style={secondaryButtonStyle} onClick={() => setStep(21)}>
                  Tilbage
                </button>
          ...
          {step === 4 && (
            <div style={stepCardStyle}>
              <InfoCheckboxCard checked={form.consent} onChange={() => update("consent", !form.consent)} infoOpen={showConsentInfo} onToggleInfo={() => setShowConsentInfo(!showConsentInfo)} label="Jeg giver samtykke til, at ByggeTalent maa opbevare og behandle mine personoplysninger i op til 6 maaneder med henblik paa rekruttering og relevante jobmuligheder. *" infoText="Dine oplysninger opbevares i op til 6 maaneder med henblik paa rekruttering og mulige fremtidige jobmuligheder. Du kan til enhver tid traekke dit samtykke tilbage." />

              <InfoCheckboxCard checked={form.gdpr} onChange={() => update("gdpr", !form.gdpr)} infoOpen={showGdprInfo} onToggleInfo={() => setShowGdprInfo(!showGdprInfo)} label="Jeg accepterer, at mine personoplysninger behandles i henhold til ByggeTalents privatlivspolitik. *" infoText="ByggeTalent behandler dine oplysninger med henblik paa rekruttering og match med relevante virksomheder. Dine data opbevares sikkert i den oplyste periode." />

              <button type="button" onClick={() => setShowPrivacyPolicy(!showPrivacyPolicy)} style={privacyLinkButtonStyle}>
              ...
              {showPrivacyPolicy && (
                <div style={privacyPolicyBoxStyle}>
                  <p style={privacyPolicyTextStyle}>
                    ...
                    1. Indledning
                    <br />
                    ByggeTalent er dataansvarlig for behandlingen af dine personoplysninger.
                    <br />
                    <br />
                    2. Hvilke oplysninger indsamler vi?
                    <br />
                    Vi indsamler oplysninger som navn, e-mail, telefonnummer, adresse, LinkedIn og CV.
                    <br />
                    <br />
                    3. Formaal
                    <br />
                    Dine oplysninger bruges til rekruttering og match med relevante virksomheder.
                    <br />
                    <br />
                    4. Opbevaring
                    <br />
                    ...
                    5. Dine rettigheder
                    <br />
                    Du kan til enhver tid faa indsigt, rettelse eller sletning af dine data.
                  </p>
                </div>
              )}

              <div style={stepButtonsStyle}>
                <button style={secondaryButtonStyle} onClick={() => setStep(3)}>
                  Tilbage
                </button>
                ...
                <button
                  style={{
                    ...primaryButtonStyle,
                    opacity: isStep4Valid ? 1 : 0.5,
                    cursor: isStep4Valid ? "pointer" : "not-allowed",
                  }}
                  onClick={() => {
                    if (isStep4Valid) setStep(5);
                  }}
          ...
          {step === 5 && (
            <div style={stepCardStyle}>
              <div style={summaryBoxStyle}>
                ...
                <SummaryRow label="Nuvaerende titel" value={form.currentTitle} />
                ...
                <SummaryRow label="Geografisk raekkevidde" value={form.distance} />
                <SummaryRow label="Valgte faglige profiler" value={`${form.profiles.length} valgt`} />
                <SummaryRow label="CV" value={cvFile ? cvFile.name : "Ikke uploadet"} />
                <SummaryRow label="Ekstra dokument" value={applicationFile ? applicationFile.name : "Ikke uploadet"} />
                <SummaryRow label="Mini-test score" value={`${totalScore}`} />
              </div>

              <div style={stepButtonsStyle}>
                <button style={secondaryButtonStyle} onClick={() => setStep(4)}>
                  Tilbage
                </button>
...
function PreviewMetric({ label, value }: { label: string; value: string }) {
  return (
    ...
    >
      <div style={{ fontSize: "12px", color: "#2C3E4F", opacity: 0.7 }}>{label}</div>
      <div style={{ fontSize: "18px", color: "#2C3E4F", fontWeight: 700, marginTop: "4px" }}>{value}</div>
    </div>
  );
}
...
function RoleSelectionCard({ title, groupedRoles, selectedValues, openGroups, onToggleGroup, onToggleRole }: { title: string; groupedRoles: Record<string, string[]>; selectedValues: string[]; openGroups: Record<string, boolean>; onToggleGroup: (group: string) => void; onToggleRole: (role: string) => void; }) {
  return (
      ...
      <div style={detailsContentStyle}>
        {Object.entries(groupedRoles).map(([group, roles]) => (
          <div key={group} style={accordionBlockLightStyle}>
            <button type="button" onClick={() => onToggleGroup(group)} style={accordionButtonLightStyle}>
              ...
              <div style={accordionContentStyle}>
                {roles.map((role) => {
                  ...
                  return (
                    ...
                    >
                      <input type="checkbox" checked={checked} onChange={() => onToggleRole(role)} />
                      <span>{role}</span>
                    </label>
                  );
                })}
...
function CvUploadCard({ cvFile, applicationFile, onCvChange, onApplicationChange }: { cvFile: File | null; applicationFile: File | null; onCvChange: (file: File | null) => void; onApplicationChange: (file: File | null) => void; }) {
  return (
    <div style={lightCardStyle}>
      <div style={cardHeadingStyle}>CV og dokumenter</div>

      <div style={detailsContentStyle}>
        <FileUploadField label="Upload CV *" file={cvFile} onChange={onCvChange} />
        <FileUploadField label="Upload ekstra dokument" file={applicationFile} onChange={onApplicationChange} />
      </div>
    </div>
  );
}
...
function FileUploadField({ label, file, onChange }: { label: string; file: File | null; onChange: (file: File | null) => void; }) {
  return (
    <div style={fileFieldStyle}>
      ...
      <input type="file" accept=".pdf,.doc,.docx" style={fileInputStyle} onChange={(e) => onChange(e.target.files?.[0] || null)} />
